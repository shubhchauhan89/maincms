...
class Security_Controller extends AppController {
    ...
    public function __construct($redirect = true) {
        ...
        if ($this->login_user && $this->login_user->role_permission) {
           
            $permissions = unserialize($this->login_user->role_permission);
            $this->login_user->role_permission = is_array($permissions) ? $permissions : array();
        } else {
            if (!$this->login_user) {
                $this->login_user = new \stdClass();
            }
            $this->login_user->role_permission = array();
        }
    }
    ...
    protected function get_access_info($group) {
        ...
        if ($this->login_user->is_admin) {
            ...
            $module_permission = get_array_value($this->login_user->role_permission, $group);
            ...
            if ($module_permission === "all") {
                ...
                $module_permission = get_array_value($this->login_user->role_permission, $group . "_specific");
    ...
    protected function access_only_allowed_members() {
        ...
        } else if (($this->module_group === "ticket" && ($this->access_type === "specific" || $this->access_type === "assigned_only")) || ($this->module_group === "lead" && $this->access_type === "own") || ($this->module_group === "client" && ($this->access_type === "own" || $this->access_type === "read_only")) || ($this->module_group === "estimate" && $this->access_type === "own")) {
    ...
    protected function can_manage_all_projects() {
        if ($this->login_user->is_admin || get_array_value($this->login_user->role_permission, "can_manage_all_projects") == "1") {
            return true;
        }
    }
    ...
    function under_construction() { 
        echo '<img src="'.site_url().'files/development.gif" alt="Under Construction" width="100%" height="600">';
    }
