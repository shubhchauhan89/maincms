...
//        log_notification("recurring_expense_created_vai_cron_job", array("expense_id" => $new_expense_id), "0");
    ...
    private function checkScheduledPosts() {
        // Retrieve scheduled posts where the scheduled time is less than or equal to the current time
        $currentDateTime = date('Y-m-d H:i:s');
        $scheduledPosts = $this->db->where('status', 'scheduled')
                                    ->where('schedule_time <=', $currentDateTime)
                                    ->get('seo_posts')
                                    ->result();
    
        // Update the status of scheduled posts to 'published'
        foreach ($scheduledPosts as $post) {
            $this->db->where('id', $post->id)
                     ->update('seo_posts', ['status' => 'published']);
        }
    
        // Update the last execution time in the database
        // $this->Settings_model->save_setting("last_cron_job_time_of_scheduled_posts_check", $currentDateTime);
    }
