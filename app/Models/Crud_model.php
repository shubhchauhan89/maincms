...
class Crud_model extends Model {
    ...
    function escape_array($values = array()) {
        if ($values && is_array($values)) {
            foreach ($values as $key => $value) {
                $values[$key] = $value ? $this->db->escapeString($value) : $value;
            }
        }
    ...
    function save_time_line($lead_id=0, $status_id=0, $owner_id=0){
        
        $db      = \Config\Database::connect();
        $builder = $db->table('thewings_time_line');
        
        $builder->select('call_type');       // names of your columns
        $builder->where('lead_id', $lead_id);                // where clause
    ...
    function get_dropdown_list($option_fields = array(), $key = "id", $where = array()) {
        ...
        // echo "<pre>"; print_r($option_fields);exit;
