                    ...
                    <div class="col-6 offset-3 text-center mt-2">
                        <div class="shadow-lg p-3 bg-white rounded">
                            ...
                            <label class="switch mb-0">
                                ...
                                <span class="slider round" onclick="topbarActionFunction()"></span>
                            </label>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#appoinmentHistory').DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": $('#url').val() + '/manage/get-appoinments',
            dom: 'Bfrtip',
            "order": [
                [1, 'asc']
            ],
        ...
        if (id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $(".loader-wrapper").hide();
                    let url = $('#url').val() + "/manage/delete-appoinment/" + id;
                    $.ajax({
                        url: url,
                        type: "delete",
                        dataType: 'json',
                        success: function(data) {
                            if (data.status) {
                                $('#appoinmentHistory').DataTable().ajax.reload()
                                $(".loader-wrapper").hide();
    ...
    function topbarActionFunction() {
        if ($("#switchCheckBox").is(':checked')) {
            var val = 'Hide';
        } else {
            var val = 'Show';
        }
        let URL = $('#url').val() + '/manage/topbar-appoiment';
        $.ajax({
            url: URL,
            type: "POST",
            data: {
