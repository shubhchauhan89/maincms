	...
	MPTT.navbar = {
		init: function init() {
			var scrollClass = "navbar-scroll",
				...
				endClass = "navbar-end",
				stickyOffset =
					$(".navbar-topbar").outerHeight() ||
					$(".navbar-top").outerHeight() ||
					100;
			$(
				'<svg xmlns="http://www.w3.org/2000/svg" width="8" height="12" fill="none"><path fill="currentColor" d="M1.47 9.47.94 10 2 11.06l.53-.53-1.06-1.06ZM6 6l.53.53.53-.53-.53-.53L6 6ZM2.53 1.47 2 .94.94 2l.53.53 1.06-1.06Zm0 9.06 4-4-1.06-1.06-4 4 1.06 1.06Zm4-5.06-4-4-1.06 1.06 4 4 1.06-1.06Z"/></svg>'
			).appendTo(".navbar-top .nav-item.navbar-dropdown > .nav-link");
			MPTT.throttleScroll(function (type, scroll) {
				// show / hide
				if ("down" === type && 500 < scroll) {
					MPTT.body.removeClass(showClass).addClass(hideClass);
				} else if ("up" === type || "end" === type || "start" === type) {
					MPTT.body.removeClass(hideClass).addClass(showClass);
				}

				if ("end" === type) {
					MPTT.body.addClass(endClass);
				} else {
					MPTT.body.removeClass(endClass);
				} // scroll class

				if ("down" === type && stickyOffset < scroll) {
					MPTT.body.addClass(scrollClass);
				}

				if ("start" === type) {
					MPTT.body.removeClass(scrollClass);
				}
			...
			MPTT.document.on("focus", ".navbar-top a", toggleShow);
			MPTT.document.on("blur", ".navbar-top a", toggleShow); // update position
