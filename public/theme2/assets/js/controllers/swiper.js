	...
	MPTT.swiper = {
		init: function init() {
			$(".swiper").each(function () {
				...
				// 			});
				// 			var text =
				// 				'<div class="container overflow-hidden"><div class="row gx-80">';

				// 			for (var i = 1; i <= total; i++) {
				// 				if (current == i) {
				// 					text +=
				// 						'<div class="col"><a href="#" class="swiper-pagination-item swiper-pagination-item-active" data-index="' +
				// 						(i - 1) +
				// 						'">' +
				// 						names[i - 1] +
				// 						"</a></div>";
				// 				} else {
				// 					text +=
				// 						'<div class="col"><a href="#" class="swiper-pagination-item" data-index="' +
				// 						(i - 1) +
				// 						'">' +
				// 						names[i - 1] +
				// 						"</a></div>";
				...
				if (dataBreakpoints) {
					...
					while (i < dataBreakpoints.split(",").length) {
						breaks[parseInt(points[i].split(":")[0], 10)] = {
							slidesPerView:
								"auto" === points[i].split(":")[1]
									? "auto"
									: parseInt(points[i].split(":")[1], 10),
						};
						i++;
					}
				...
				swiper.on("slideChangeTransitionStart", function () {
					MPTT.document.trigger("swiperSlideChangeTransitionStart");
				});
				swiper.on("touchMove", function (event) {
					MPTT.document.trigger("swiperTouchMove", [
						event.clientX,
						event.clientY,
					]);
				});
